...
- **One value for the whole entity.** When an attribute is stable for the entity you are editing, it holds a single value across *all* of the entity's annotations (for example, the views of the same object across multiple cameras), instead of being set separately on each frame or view. Editing it once updates it for the entity everywhere.
...
- **Changing a value moves it.** Editing a stable attribute to a new value moves that value onto the annotation you are currently editing and clears the stale copies on the entity's other annotations. Leaving the value unchanged leaves it wherever it already lives.
...
- **Promote by clicking.** Click a revealed shadow entity to promote it back into the active set — it becomes fully visible, appears in the entities list, and will be saved on submit. This affects only the annotation you clicked; the entity's other annotations are untouched. Promotion is a one-way action within your session.
