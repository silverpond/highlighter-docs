...
- **Setting a value becomes the entity's value for that stability period.** When you edit the attribute on the view you are working on and choose a value, that value becomes the entity's value for the stability window. Any *other* views that held a **conflicting** (different) value within that window are cleared, so the conflict is resolved down to your new value. Views that already held the **same** value do not conflict and are left as they are. Leaving the value unchanged leaves it on the annotation where it was originally set.
