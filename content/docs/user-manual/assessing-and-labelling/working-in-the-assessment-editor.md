...
The timeline bar at the bottom of the editor shows the current time period as a start and end time. Click either time to open the Time Range dialog and type specific dates and times, or use the chevron buttons beside the times to step back ('‹') or forward ('›') by one whole period. Each click shifts the window by its own duration (keeping periods contiguous) and reloads the tracks for the new window.
