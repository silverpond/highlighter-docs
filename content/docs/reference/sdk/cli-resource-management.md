...
# Get one case's resource information; no files are downloaded
hl case get --id <CASE_ID>

# List the cases in a workflow order
hl case list --workflow-order-id <ORDER_ID> --limit 25

# Export a case: download its files into <OUTPUT_DIR>/<CASE_ID>/
...
`case get` reports the case's data files (including their UUIDs) and tasks,
which `case create` needs as input. `case export` downloads payload files,
selectable by content type and time window. `case copy` re-creates a case over
the same data files — useful for re-running an agent without disturbing the
original. `case read` remains as a deprecated alias of `case get`.
