...
Connectors can now subscribe to a **Case Ready** event, which fires once when a case moves from draft to ready. HTTP Request connectors receive a versioned JSON document describing the case, including its workflow, entities, data sources, latest submission, and file links. Idempotency headers let your endpoint safely ignore repeated deliveries. You can limit the event to particular workflows or workflow orders. See [Case Ready Webhook](../../user-manual/managing-workflows/connectors/#case-ready-webhook).
...
- **Authenticated webhooks.** HTTP Request connectors can send a bearer token. The token is stored encrypted, is never displayed again, and requires an `https` address — there is no exemption for local endpoints.
...
- Shadowed entities are not saved on submit unless promoted, so they do not carry forward to later stages.
